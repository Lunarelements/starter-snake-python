    ...
    for move in other_moves:
        if future_head == move.coordinates():
            ...
            if other["length"] >= you["length"]:
                # TODO - make the snake take a chance(score) to hit other snake if the other moves run into wall or
                #  itself
                print(f'Snake\'s length is {you["length"]}, theirs is {other["length"]}. This possible collision is '
                      f'last resort.')
                return -10
            return -0.5
    return 0
...
def generate_possible_moves(body, width, height):
    ...
    for next_move in POSSIBLE_MOVES:
        ...
        if future_position != neck and 0 <= future_position["x"] < width and 0 <= future_position["y"] < height:
...
def find_room(board, board_grid, body):
    rooms = []
    moves = generate_possible_moves(body, board["width"], board["height"])
    for move in moves:
...
def strategy_normal(board, board_grid, head, body):
    find_food(board, board_grid, head)
    find_room(board, board_grid, body)
